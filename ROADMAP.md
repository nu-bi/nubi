...
|---|---|---|
| **Cloud warehouses** (BigQuery, Snowflake, Redshift, Databricks SQL) | First-class | Sweet spot. Per-dialect rewriter + Arrow path each. Native-Arrow routes differ per vendor. |
| **OLTP** (Postgres incl. **Neon**, MySQL) | Supported | "Source unbounded, pushed to warehouse" assumption weakens — these *are* the source of truth; want a hot mirror for heavy analytics. |
| **Fast OLAP** (ClickHouse, DuckDB-as-service, Trino) | Supported | Trino is itself federation — decide push-down-to vs compete. |
