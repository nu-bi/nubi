...
Closes one billing cycle for one organisation:
    ...
    5. Collect the total from the saved Paystack card (in ZAR, at the live FX
       rate — prices are anchored in USD).
    6. Render the invoice PDF, persist the invoice, and email it to the customer.
    7. Record billing events + advance the subscription period.
        ...
        if amount > 0:
            items.append(
                InvoiceLineItem(
                    ...
                    amount, kind="overage",
                    quantity=Decimal(str(int(over_cu))), unit="1k CU",
        ...
        if amount > 0:
            items.append(
                InvoiceLineItem(
                    ...
                    amount, kind="overage",
                    quantity=Decimal(str(int(over_embed))), unit="10k sess",
...
async def run_billing_cycle(
    *,
    ...
    # 5. Collect from the saved Paystack card.
    charge_result: dict[str, Any] = {"status": "skipped"}
    if collect and invoice.total_zar > 0:
        charge_fn = charge_fn or _default_charge_fn
        ref = f"nubi-inv-{invoice.invoice_number}".lower().replace(" ", "")
...
async def _apply_wallet_credit(
    ...
    the ZAR equivalent of the cents drawn.  Idempotent per period via ``ref_id``.
    """
    try:
...
def _add_one_month(dt: datetime) -> datetime:
    """Return *dt* advanced by ~one month (clamped to end-of-month)."""
    year = dt.year + (1 if dt.month == 12 else 0)
    month = 1 if dt.month == 12 else dt.month + 1
    # Clamp day to a safe 28 to avoid month-length overflow.
    day = min(dt.day, 28)
